...
{
    public sealed class AmazonS3Emulator : IAmazonS3
    {
        private readonly HttpClient _httpClient;

        public AmazonS3Emulator(HttpClient httpClient)
        {
            _httpClient = httpClient;
        }

        public async Task<PutObjectResponse> PutObjectAsync(PutObjectRequest request, CancellationToken cancellationToken = new CancellationToken())
        {
            using var message = new HttpRequestMessage
            {
                Method = HttpMethod.Post,
                Content = new StringContent(request.ContentBody, Encoding.UTF8, "application/json")
            };
        ...
        #region Unsupported methods in simulator
...
#pragma warning restore CA1055 // Uri return values should not be strings
        ...
        public Task<GetObjectLockConfigurationResponse> GetObjectLockConfigurationAsync(GetObjectLockConfigurationRequest request,
            CancellationToken cancellationToken = new CancellationToken())
        {
            throw new NotImplementedException();
        }
        ...
        #endregion Unsupported methods in simulator
    }
}
