...
namespace FinanceChargesListener.UseCase
{
    public class EstimateActualFileProcessUseCase : IEstimateActualFileProcessUseCase
    {
           ...
           string createdBy, short chargeYear)
        {
            ...
            {
                var blockAsset = assetListResult.FirstOrDefault(x => x.AssetId == item.Key);
                Guid id;
                if (blockAsset != null)
                {
                    id = blockAsset.Id;
                ...
                var blockItem = new EstimateActualCharge
                {
                    AssetId = id,
                ...
                chargesResultList.Add(ChargeHelper.GetChargeModel(assetType,
                    ChargeGroup.Leaseholders, chargeSubGroup, createdBy, chargeYear, blockItem));
