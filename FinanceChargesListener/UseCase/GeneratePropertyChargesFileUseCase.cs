...
namespace FinanceChargesListener.UseCase
{
    public class GeneratePropertyChargesFileUseCase : IGeneratePropertyChargesFileUseCase
    {
        ...
        public async Task ProcessMessageAsync(EntityEventSns message, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            var fileName = $"PropertyCharges{DateTime.Now:yyyyMMddHHmmssfff}.csv";
                        ...
                        if (recordsCount > 0)
                        {
                            try
                            {
                                estimatesActual.Add(new EstimateActualCharge
                                {
                                    ...
                                    PropertyAddress = reader.GetValue(2)?.ToString()?.Replace(",", " "),
                                    TenureType = reader.GetValue(3)?.ToString()?.Replace(",", " "),
                                    Name1 = reader.GetValue(8)?.ToString()?.Replace(",", " "),
                                    AddressLine1 = reader.GetValue(9)?.ToString()?.Replace(",", " "),
                                    AddressLine2 = reader.GetValue(10)?.ToString()?.Replace(",", " "),
                                    AddressLine3 = reader.GetValue(11)?.ToString()?.Replace(",", " "),
                                    AddressLine4 = reader.GetValue(12)?.ToString()?.Replace(",", " "),
