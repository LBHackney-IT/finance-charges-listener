...
terraform {
  backend "s3" {
    ...
    encrypt = true
    region  = "eu-west-2"
    key     = "services/finance-charges-listener/state"
  }
}
...
#
data "aws_ssm_parameter" "charges_sns_topic_arn" {
  name = "/sns-topic/production/charges/arn"
}

### This is the definition of the dead letter queue used whem message processsing fails for a given message
#
resource "aws_sqs_queue" "charges_dead_letter_queue" {
  name                        = "chargesdeadletterqueue.fifo"
  fifo_queue                  = true
...
#
resource "aws_sqs_queue" "charges_queue" {
  ...
  redrive_policy              = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.charges_dead_letter_queue.arn,
...
resource "aws_sqs_queue_policy" "charges_queue_policy" {
  ...
  policy = <<POLICY
  {
      ...
      "Statement": [
      {
          ...
          "Resource": "${aws_sqs_queue.charges_queue.arn}",
          "Condition": {
          "ArnEquals": {
              "aws:SourceArn": "${data.aws_ssm_parameter.charges_sns_topic_arn.value}"
          }
          }
      }
      ]
...
resource "aws_sns_topic_subscription" "charges_queue_subscribe_to_charges_sns" {
  ...
  protocol  = "sqs"
  endpoint  = aws_sqs_queue.charges_queue.arn
...
resource "aws_ssm_parameter" "charges_sqs_queue_arn" {
  name  = "/sqs-queue/production/charges/arn"
  type  = "String"
  value = aws_sqs_queue.charges_queue.arn
}
